...
def append_json(file, obj, is_loja=False):
    with open(file, "r+", encoding='utf-8') as f:
        ...
        while pos > 0:
            ...
            if f.read(1) == ']':
                f.seek(pos)
                if is_loja:
                    f.write(',\n        ' + json.dumps(obj, indent=4).replace('\n', '\n        ') + '\n    ]\n}')
                else:
                    f.write(',\n    ' + json.dumps(obj, indent=4).replace('\n', '\n    ') + '\n]')
                break
...
def handle_client(conn):
    try:
        ...
        if action == 'get_all_users':
            ...
            append_json('utilizadores.json', req.get('data'), is_loja=False)
            resp = {"status": "ok"}
            ...
            append_json('dados.json', loja_nova, is_loja=True)
